...
parser.add_argument("--model", default="snn")
...
class SNN(nn.Module):
    ...
    def forward(self, x):
        ...
        weights = torch.matmul(x, self.attention.T)
...
def main():
    ...
    model = SNN(input_dim=1+train_dataset.seq_embedding.shape[1]+train_dataset.seq_coin_embedding.shape[1])
